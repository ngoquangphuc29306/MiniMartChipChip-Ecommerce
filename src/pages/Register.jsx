...
const Register = () => {
  ...
  // Hover/indicator state for animated pill movement - copied from Login.jsx
  ...
  const updateIndicator = () => {
    // Current active is register
    const active = hoveredTab ?? 'register';
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password.length < 6) {
      toast({
        ...
        description: "Mật khẩu phải có ít nhất một chữ thường, một chữ hoa, một số và một ký hiệu."
      });
      return;
    }
    ...
    if (!error) {
      // Set flag to show welcome promo banner on first login
  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col bg-background dark:bg-black group-design-root overflow-x-hidden">
      <Helmet>
        <title>Đăng ký - Minimart ChipChip</title>
      </Helmet>
      ...
      <div className="layout-container flex h-full grow flex-col">
        <div className="flex flex-1 justify-center items-center p-4 sm:p-6 md:p-8">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' }} className="flex flex-col lg:flex-row w-full max-w-6xl overflow-hidden rounded-[2rem] bg-card dark:bg-zinc-900 shadow-2xl border border-border">
            ...
            {/* Image Side - Left for consistency with Login or we can flip it? 
               Login.jsx has Image on Left. Let's keep Image on Left for perfect consistency. */}
            <div className="w-full lg:w-1/2 p-10 lg:p-12 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/10 dark:to-orange-900/10 flex-col justify-center items-center hidden lg:flex">
              <div className="flex flex-col items-center text-center">
                <img
                  ...
                  loading="lazy"
                />
                <h2 className="text-3xl font-display font-bold mt-8 text-foreground">Tham gia cùng ChipChip</h2>
                <p className="mt-4 text-muted-foreground max-w-md">Tạo tài khoản ngay hôm nay để nhận nhiều ưu đãi hấp dẫn và trải nghiệm mua sắm tuyệt vời.</p>
              </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 p-8 sm:p-10 md:p-12 flex flex-col justify-center">
              <div className="layout-content-container flex flex-col w-full max-w-md mx-auto">
                ...
                <h1 className="text-foreground tracking-tight text-[32px] font-bold leading-tight text-center pb-1">Tạo tài khoản mới</h1>
                <p className="text-muted-foreground text-base font-normal leading-normal pb-6 text-center">Điền thông tin của bạn để đăng ký.</p>
                ...
                <div className="flex w-full py-3 justify-center">
                  <div ref={indicatorContainerRef} className="relative flex h-12 items-center justify-center rounded-full bg-muted/40 p-1.5 shadow-sm border border-border">
                    ...
                    <label
                      ...
                      className={`relative z-20 flex cursor-pointer h-10 items-center justify-center overflow-hidden rounded-full px-6 text-sm font-medium leading-normal transition-all duration-300 ${isRegister ? 'text-muted-foreground hover:text-foreground' : 'text-foreground font-bold'}`}>
                    ...
                    <label
                      ...
                      className={`relative z-20 ml-3 flex cursor-pointer h-10 items-center justify-center overflow-hidden rounded-full px-6 text-sm font-medium leading-normal transition-all duration-300 ${isRegister ? 'text-foreground font-bold' : 'text-muted-foreground hover:text-foreground'}`}>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
                  <label className="flex flex-col w-full">
                    <p className="text-foreground text-base font-medium leading-normal pb-2">Họ và tên</p>
                    <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} required placeholder="Nguyễn Văn A" className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-foreground focus:outline-0 focus:ring-2 focus:ring-yellow-400/50 border border-border bg-muted/50 focus:bg-background h-14 placeholder:text-muted-foreground p-4 text-base font-normal leading-normal transition-all" />
                  </label>
                  ...
                  <label className="flex flex-col w-full">
                    <p className="text-foreground text-base font-medium leading-normal pb-2">Số điện thoại</p>
                    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="09xxx..." className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-foreground focus:outline-0 focus:ring-2 focus:ring-yellow-400/50 border border-border bg-muted/50 focus:bg-background h-14 placeholder:text-muted-foreground p-4 text-base font-normal leading-normal transition-all" />
                  </label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label className="flex flex-col w-full">
                      ...
                      <div className="relative flex w-full flex-1 items-stretch">
                        ...
                        <button type="button" onClick={() => setShowPassword(v => !v)} className="text-muted-foreground absolute inset-y-0 right-0 flex items-center justify-center px-4 hover:text-foreground">
                    ...
                    <label className="flex flex-col w-full">
                      <p className="text-foreground text-base font-medium leading-normal pb-2">Xác nhận</p>
                      <div className="relative flex w-full flex-1 items-stretch">
                  ...
                  <div className="pt-4">
                    <Button type="submit" disabled={loading} className="flex w-full items-center justify-center rounded-xl bg-yellow-400 h-14 px-6 text-base font-bold text-white shadow-lg shadow-yellow-400/20 hover:bg-yellow-500 transform transition-all duration-200 hover:-translate-y-1">{loading ? 'Đang xử lý...' : 'Đăng ký ngay'}</Button>
                  </div>
                </form>
